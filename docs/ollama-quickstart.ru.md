# Rejudge на Ollama — по шагам

*[In English](ollama-quickstart.md)*

От нуля до работающего ревью. Каждый шаг — команда и то, что должно появиться в ответ.
Если появилось другое, смотрите «Если не сходится» в конце нужного шага.

Восемь шагов, минут на всё — десять. Шаги 1–6 проверены целиком в чистой песочнице:
свежий клон, пустые каталоги настроек, отдельный npm-префикс.

## Сначала — как это устроено

Три вещи, без которых шаги ниже читаются неправильно.

**Ставится один раз и глобально.** `rejudge` — команда, как `git` или `curl`. Она не зависимость
проекта: ни в `package.json`, ни в `node_modules` не попадает, и ставить её в каждый репозиторий
не нужно.

**В проект ничего линковать не надо.** Связь с проектом — это текущий каталог, и всё.
Заходите в репозиторий и запускаете `rejudge`. Каталог решает две вещи: откуда взять конфиг и
какие файлы смогут прочитать ревьюеры.

**Настройка тоже одна на машину.** `rejudge setup ollama` пишет два файла в вашем домашнем
каталоге — провайдер для Pi и панель. В проект он не пишет ничего.

Единственное, что вообще бывает внутри проекта, — необязательный `.rejudge/config.json`, если
в этом конкретном репозитории нужна *другая* панель. Это обычный файл, а не линк, и шаг 7
показывает, как его получить.

---

## Шаг 1. Node 22 или новее

```bash
node --version
```

Ждём `v22.19.0` или выше.

Если меньше:

```bash
nvm install 22
nvm use 22
```

**Если не сходится.** Проверьте ту же команду в новом окне терминала: `nvm use` живёт только в текущей сессии. Чтобы закрепить — `nvm alias default 22`. Если другие проекты держатся на старой Node, дефолт не трогайте: шаг 4 поставит обёртку, которая сама найдёт нужную версию.

---

## Шаг 2. Ollama и вход в аккаунт

```bash
ollama --version
ollama signin        # если ещё не входили
```

Проверяем, что облако отвечает по подписке:

```bash
ollama run glm-5.2:cloud "ping"
```

Ждём осмысленный ответ модели. Это же поднимает демон, если он не был запущен.

**Если не сходится.** `402 extra usage only` — модель вне вашего тарифа, возьмите другую из шага 3. Ошибка авторизации — повторите `ollama signin`.

---

## Шаг 3. Стянуть модели

Нужно **минимум четыре** модели: три ревьюера и судья. Берите из разных лабораторий —
две модели одной линейки ошибаются одинаково, и панель из них ничего не проверяет.

Каталог того, что умеет и рассуждать, и вызывать инструменты:
<https://ollama.com/search?c=thinking&c=cloud&c=tools>

```bash
ollama pull deepseek-v4-flash:0731-cloud
ollama pull minimax-m3:cloud
ollama pull gemma4:31b-cloud
ollama pull glm-5.2:cloud
```

**Правило имён.** У модели без тега — `<модель>:cloud`. С тегом — `<модель>:<тег>-cloud`.
То есть `glm-5.2:cloud`, но `gpt-oss:120b-cloud`.

Проверяем:

```bash
ollama list
```

**Если не сходится.** `model not found` — почти всегда имя: перепутан `:cloud` и `-cloud`.
Стягивать облачные модели, кстати, не обязательно — демон проксирует их и так — но
стянутая заглушка (несколько сотен байт) кладёт модель в локальный список, откуда её видит
шаг 5.

---

## Шаг 4. Сделать `rejudge` глобальной командой

Клонируем в постоянное место — это чекаут инструмента, а не часть проекта, который вы
собираетесь ревьюить. Например в `~/work`:

```bash
git clone -b mine https://github.com/pksmall/rejudge.git ~/work/rejudge
cd ~/work/rejudge
just setup
npm link
```

Нужны `bun` и `just` — `brew install bun just`.

`npm link` **без аргументов** значит «сделать этот чекаут глобальной командой `rejudge`».
Не путайте с `npm link <имя>`, которую запускают *внутри* проекта: та делает обратное,
и здесь она не нужна. Ни один проект в этом шаге не участвует.

Проверяем — из любого каталога, лишь бы не из чекаута:

```bash
cd ~ && rejudge --help | head -3
```

Ждём строку `usage: rejudge`, а ниже в выводе — `setup ollama`. Если команда нашлась из
домашнего каталога, значит она глобальная и в остальных проектах тоже найдётся.

**Если не сходится.** `rejudge: command not found` — каталог `bin` глобального npm не в `PATH`.
Он печатается командой `npm prefix -g`, добавьте к нему `/bin`.

`webidl.util.markAsUncloneable is not a function` — запустилась старая Node. Вернитесь к шагу 1
либо положите обёртку, которая сама выбирает интерпретатор:

```bash
mkdir -p ~/.local/bin
cat > ~/.local/bin/rejudge <<'EOF'
#!/bin/sh
for dir in $(ls -d "$HOME"/.nvm/versions/node/v2[2-9]* "$HOME"/.nvm/versions/node/v[3-9]* 2>/dev/null | sort -Vr); do
    if [ -f "$dir/lib/node_modules/rejudge/bin/rejudge.js" ]; then
        exec "$dir/bin/node" "$dir/lib/node_modules/rejudge/bin/rejudge.js" "$@"
    fi
done
echo "rejudge: no Node >= 22 with the rejudge package installed" >&2
exit 1
EOF
chmod +x ~/.local/bin/rejudge
```

`~/.local/bin` должен быть в `PATH` раньше каталога Node.

Можно поставить и из npm — `npm install -g rejudge` — но тогда команды `setup` не будет,
она есть только в этом форке, и шаг 5 придётся делать руками по [ollama.md](ollama.md).

---

## Шаг 5. Одна команда настройки, один раз на машину

Запускать можно откуда угодно — она пишет только в домашний каталог:

```bash
rejudge setup ollama --dry-run     # посмотреть, ничего не записывая
rejudge setup ollama               # записать
```

Ждём примерно такое — числа будут ваши, они просто отражают ваш список моделей:

```
Ollama setup — 4 models declared, 0 skipped

provider  wrote /Users/you/.pi/agent/models.json
config    wrote /Users/you/.config/rejudge/config.json

panel
  reviewer  ollama/minimax-m3:cloud@high
  reviewer  ollama/gemma4:31b-cloud@high
  reviewer  ollama/glm-5.2:cloud@high
  judge     ollama/deepseek-v4-flash:0731-cloud@high
  every slot comes from a different lab
```

Команда не ходит в облако и ничего не пуллит: она объявляет то, что уже лежит у вас
локально. Список моделей — ваш, и обновлять его тоже вам.

**Если не сходится.**

- `a panel needs 4 eligible models … found 2` — вернитесь к шагу 3, моделей мало.
- Числа больше, чем в примере, и есть блок `skipped` — так и должно быть, если у вас лежат
  и другие модели, кроме четырёх из шага 3. Команда объявляет всё пригодное, а непригодное
  перечисляет с причиной: обычно нет `thinking` или нет `tools`. Это не ошибка.
- `some slots reuse the same lab` — панель собралась, но из одной линейки. Работать будет,
  проверять — нет. Стяните модель другого вендора.
- Строка `local models` — в панель попали локальные веса. Прочитайте про контекст в
  [ollama.md](ollama.md#local-models): демон отдаёт не то окно, что заявлено в модели,
  а переполнение обрезается молча.
- `cannot reach the Ollama daemon` — демон не поднят, вернитесь к шагу 2.

---

## Шаг 6. Первый настоящий прогон

Сначала пустяк, из любого каталога:

```bash
rejudge "Reply with the single word READY and nothing else."
```

В stderr идёт прогресс с именами моделей, в stdout — ответ. Занимает 20–40 секунд.
Последняя строка stderr даёт `run id` для продолжения разговора.

Теперь на реальной задаче. **Вот и весь способ «подключить» Rejudge к проекту** — просто зайти
в него. Никакой установки в проект не было и не будет:

```bash
cd ~/your/project
git diff | rejudge "review this change"
```

Запускайте из корня того репозитория, который хотите отдать на ревью: инструменты ревьюеров
работают в текущем каталоге, и из соседнего они ваш код не увидят.

**Если не сходится.**

- `no config found` — шаг 5 не записал конфиг. Проверьте, что не запускали его с `--dry-run`.
- `410 … was retired` — модель снята с облака. `ollama list` её всё ещё показывает, но
  сервис больше не отдаёт: стяните замену (шаг 3) и повторите шаг 5.
- `invalid reasoning value` — в конфиге уровень, которого Ollama не знает. Допустимы
  `low`, `medium`, `high`, `xhigh`; `minimal` она отвергает.
- Ревью пришло, но задачу проигнорировало — в `models.json` у модели нет
  `compat.supportsDeveloperRole: false`. Перезапустите шаг 5, он ставит его сам.
- Прогон висит несколько минут на одной модели — бывает: панель ждёт самую медленную.
  `nemotron-3-super` в замере взял 13 минут против 2 у остальных.

---

## Шаг 7. Научить агента звать Rejudge

До сюда Rejudge — ваша команда в терминале. Чтобы им пользовался кодовый агент, нужно
по-разному в зависимости от агента, и разница ровно одна: **у Pi есть родной инструмент,
у всех остальных — только скиллы плюс та же CLI.**

### Pi — родной инструмент и воркфлоу

```bash
pi install "$(npm root -g)/rejudge"
```

Pi запоминает путь, а не копирует, так что команда, инструмент `rejudge` и оба воркфлоу
(`/rejudge`, `/rejudge-diff`) берутся из одной установки. Перезапустите Pi, если он был открыт.
Внутри Pi агент видит живой прогресс по стадиям и получает над прогоном больше контроля, чем
над CLI.

### Claude Code — скиллы

Скиллы лежат в `~/.claude/skills/<имя>/SKILL.md`. Ставим симлинками на чекаут, чтобы они
обновлялись вместе с ним:

```bash
cd ~/work/rejudge
mkdir -p ~/.claude/skills
ln -s "$PWD/docs/skills/rejudge" ~/.claude/skills/rejudge
ln -s "$PWD/docs/skills/rejudge-diff" ~/.claude/skills/rejudge-diff
```

Появятся `/rejudge` и `/rejudge-diff`. Первый — любой вопрос на панель, второй — код-ревью
текущего диффа с разбором по P0–P3.

### Остальные агенты — OpenCode, Codex и любой другой

Родного инструмента нет ни у кого, кроме Pi, так что вариантов два.

Если агент умеет читать Agent Skills — поставьте их через Skills CLI:

```bash
npx skills add syabro/rejudge -g -y
```

Это отдельная копия из апстрима, поэтому обновлять её надо самому:
`npx skills update -g -y`. Учтите: в апстриме нет команды `setup`, но скиллу она и не нужна —
он вызывает `rejudge` для ревью, а настройка у вас уже сделана шагом 5.

Если агент про скиллы не знает, но умеет запускать команды — этого достаточно. Скажите ему
запускать `rejudge` в корне проекта и читать ответ из stdout; сам файл
[`docs/skills/rejudge/SKILL.md`](skills/rejudge/SKILL.md) можно скормить ему как инструкцию —
там написано, как звать, чего не делать и как показывать результат. Для Codex в нём есть
отдельный абзац про запуск в TTY.

**Что важно для любого агента.** Прогон длится минуты — ставьте щедрый таймаут и не уводите
в фон. Ревьюеры работают read-only; флаги `--unsafe` / `--full` дают им запись и bash, и без
явной причины их передавать не надо.

---

## Шаг 8. По желанию

**Своя панель в отдельном проекте.** Единственное, что вообще кладётся внутрь проекта, и то
по желанию: файл `<проект>/.rejudge/config.json`. Он перебивает глобальный — молча, поэтому при
неожиданном результате сверяйтесь со строкой `config:` в выводе. Написать его можно руками или
командой из корня проекта:

```bash
cd ~/your/project
rejudge setup ollama --project        # панель в .rejudge/config.json этого проекта
```

Это по-прежнему не установка Rejudge в проект — только файл с настройками.

**Логи рассуждений.** `"debugLog": true` в конфиге — каждый прогон пишет
`.rejudge/logs/*.jsonl`. Держите вне git.

**Обновить модели.** После каждого `ollama pull` или `ollama rm` перезапустите
`rejudge setup ollama` — он перепишет провайдера под новый список. Конфиг при этом не
тронется, для замены панели нужен `--force`.

---

## Что важно помнить

Глобальная команда — симлинк на этот чекаут, потому что ставили через `npm link`. Значит она
следует за текущей веткой: переключите чекаут на `upstream/main`, и `setup ollama` исчезнет,
в апстриме его нет. После правок в `src/` нужна пересборка — `just build`.

Ключей нигде не нужно: облачные модели идут через локальный демон, а креды держит он.

Подробное объяснение каждой настройки, правило имён, таблица отказов и ловушка с локальным
контекстом — в [ollama.md](ollama.md).
